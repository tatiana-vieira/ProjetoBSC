...
altpdipro_route = Blueprint('altpdipro', __name__)

@altpdipro_route.route('/selecionar_programapro', methods=['GET', 'POST'])
def selecionar_programapro():
    if request.method == 'POST':
        pdi_id = request.form['pdi_id']
        return redirect(url_for('altpdipro.exibir_altpdipro', pdi_id=pdi_id))

    pdis = PDI.query.all()
    return render_template('selecionar_programapro.html', pdis=pdis)

@altpdipro_route.route('/exibir_altpdipro')
def exibir_altpdipro():
    pdi_id = request.args.get('pdi_id')
    if not pdi_id:
        return redirect(url_for('altpdi.selecionar_programapro'))

    pdi_data = PDI.query.filter_by(id=pdi_id).first()
    if not pdi_data:
        flash('Programa não encontrado.')
        return redirect(url_for('altpdipro.selecionar_programapro'))
...
@altpdipro_route.route('/selecionar_planejamentopro', methods=['GET', 'POST'])
def selecionar_planejamentopro():
    if request.method == 'POST':
        pdi_id = request.form['pdi_id']
        return redirect(url_for('altpdipro.exibir_planejamentopro', pdi_id=pdi_id))

    pdis = PDI.query.all()
    return render_template('selecionar_planejamentopro.html', pdis=pdis)

@altpdipro_route.route('/exibir_planejamentopro')
def exibir_planejamentopro():
    pdi_id = request.args.get('pdi_id')
    if not pdi_id:
        return redirect(url_for('altpdipro.selecionar_planejamentopro'))
    ...
    return render_template('exibir_planejamentopro.html', pdi=pdi_data, objetivos=objetivos, metas=metas, indicadores=indicadores)

###################################################################################################################################3
@altpdipro_route.route('/export_pdi/excel')
@login_required
...
@altpdipro_route.route('/export_pdi/pdf')
@login_required
def export_pdi_pdf():
    pdi_data = PDI.query.all()
    objetivos = Objetivo.query.filter(Objetivo.pdi_id.in_([pdi.id for pdi in pdi_data])).all()
