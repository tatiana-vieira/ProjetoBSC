    ...
    if session.get('role') == 'Coordenador':
        ...
        if planejamento_selecionado_id:
            ...
            if acoespe:
                try:
                    # Criando DataFrame
                    df = pd.DataFrame([(acao.nome, acao.data_inicio, acao.data_termino) for acao in acoespe],
                                      columns=['Ação', 'Início', 'Término'])
                    ...
                    df = df.dropna(subset=['Início', 'Término'])
                    ...
                    ax.set_title('Progresso de Execução por Ação')
                    ...
                    status_counts = pd.Series([acao.status for acao in acoespe if acao.status]).value_counts()
                    ...
                    if not df.empty:
                        ...
                        flash('Não há dados suficientes para o gráfico de duração média.', 'warning')

                except Exception as e:
                    flash(f'Erro ao gerar os gráficos: {str(e)}', 'danger')

        return render_template('gantt.html', planejamentos=planejamentos,
                               planejamento_selecionado=planejamento_selecionado,
                               acoes=acoespe, plot_urls=plot_urls)
